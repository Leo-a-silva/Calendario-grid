...
export function AppointmentModal({ open, onOpenChange }: AppointmentModalProps) {
  ...
  const handleDniSearch = () => {
    const patient = mockPatients.find(p => p.dni === searchedDni);
  ...
  const onSubmit = (data: AppointmentFormData) => {
    if (!foundPatient) {
      ...
      const newPatient: Patient = {
        ...
        dni: data.dni,
        numeroHistoriaClinica: `HC${(mockPatients.length + 1).toString().padStart(3, '0')}`,
        sexo: data.sexo,
      ...
      enqueueSnackbar(`Nuevo paciente ${data.nombre} ${data.apellido} creado y turno agendado`, { 
        variant: 'success',
